    ...
    try:
        ...
        if len(sys.argv) > 1:
            ...
            text = text.replace("#", "") if text.startswith(
                "#") else f"can you please draw system diagram of {text} with explanation "
            # todo without image
            sys.argv.clear()
            exit(1)

        if strKey == "Key.media_play_pause":
            text = "can you solve this question in java with most efficient time and space complexity with explanation"
            # text = "draw system design of this requirements with explanation"
            startMicLoop(text)
